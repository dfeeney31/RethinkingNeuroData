...
createPowers <- function(baselinePwr, meanChg, sdChg){
  
# Baseline power for subjects and return 20 observations 
  pwrList <- numeric(noObs)
  for (i in 1:sampleSize){
    tmpStart <- ((i-1)*noObs) + 1
    tmpEnd <- (noObs) * i
    pwrList[tmpStart:tmpEnd] <- baselinePwr[i] + rnorm(noObs, mean = meanChg, sd = sdChg)
  }
...
testAnovaRand <- function(metric, df) {
  
  myformula <- as.formula(paste0(metric," ~ cond", " + (cond|sub)"))
  myformula2 <- as.formula(paste0(metric, " ~ (1|sub)"))
  
  full.mod = lmer(myformula, data = df, REML = TRUE, na.action = "na.omit" )
  red.mod = lmer(myformula2, data = df, REML = TRUE, na.action = "na.omit" )
  
  conditions.emm <- emmeans(full.mod, "cond", lmer.df = "satterthwaite")
  #conditions.emm
  contrast(conditions.emm, "trt.vs.ctrl", ref = "seated") 
  
  
  newList <- list("randEffectMod" = summary(full.mod), "anovaBetweenMods" = anova(full.mod, red.mod),
                  "contrasts" = conditions.emm, "Contrasts2" = contrast(conditions.emm, "trt.vs.ctrl", ref = "seated"))
  return(newList)
...
```{r}
# standard anova without accounting for within subjects has supremely low p-value
